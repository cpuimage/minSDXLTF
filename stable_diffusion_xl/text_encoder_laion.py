...
class TextEncoderLaion(tf.keras.Model):
    def __init__(self, max_length=77, embed_dim=1280, vocab_size=49408, num_heads=20, num_layers=32, name=None,
        ...
        pooled_output = tf.gather_nd(
            params=embedded,
            indices=tf.stack(
                values=(tf.range(batch_size, dtype=tf.int64), tf.math.argmax(tokens, axis=-1)), axis=1
            ),
        )
        proje_out = tf.keras.layers.Dense(1280, name="text_projection", use_bias=False)(pooled_output)
        super().__init__([tokens, positions], [out[-2], proje_out], name=name)
