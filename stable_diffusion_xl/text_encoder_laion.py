...
def gelu(x):
    coeff = ops.cast(0.044714998453855515, x.dtype)
    return 0.5 * x * (1.0 + ops.tanh(0.7978845608028654 * (x + coeff * ops.power(x, 3))))


class TextEncoderLaionProj(Model):
    def __init__(self, embed_dim=1280, name=None, ckpt_path=None, lora_dict=None):
        embedded = layers.Input(shape=(embed_dim,), dtype="float32", name="embedded")
        proje_out = layers.Dense(1280, name="text_projection", use_bias=False)(embedded)
        ...
        if ckpt_path is not None:
            if os.path.exists(ckpt_path):
                ...
                origin = ckpt_path
        model_weights_fpath = utils.get_file(origin=origin, fname="text_encoder2.fp16.safetensors")
...
class TextEncoderLaion(Model):
    def __init__(self, max_length=77, embed_dim=1280, vocab_size=49408, num_heads=20, num_layers=32, name=None,
        ...
        for idx in range(num_layers):
            ...
            out.append(x)
        embedded = layers.LayerNormalization(epsilon=1e-5, name="text_model.final_layer_norm")(out[-1])
